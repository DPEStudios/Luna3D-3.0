...
| 2026-07-08 | S2 | COMPLETADA | Saneamiento ejecutado el mismo dia de emision del plan: 15 docs aplicados/superados movidos a docs/historico/ (con README indice); carpeta Drive archivada como _OBSOLETO_Web_Luna3D_v3_archivado_2026-07 + LEEME; banner de vigencia en CONTEXTO_BASE; referencias corregidas en CLAUDE.md y MARCA_referencia; memoria actualizada (INDEX + estrella3d). Commit/push realizados el 2026-07-08 junto con la publicacion de S1 — cerrada. |
| 2026-07-08 | S1 | COMPLETADA | Hardening ejecutado: vercel.json con bloque global (X-Content-Type-Options nosniff, X-Frame-Options SAMEORIGIN, Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy camara/mic/geo/pago vacios) + CSP. Inventario de origenes: supabase.co del proyecto (connect+img), fonts.googleapis.com (style), fonts.gstatic.com (font), api.web3forms.com (connect); scripts solo self + unsafe-inline. CSP partio Report-Only, 0 violaciones en flujo completo (home, catalogo, ficha, carrito, cuenta, contacto, legal, seguimiento, nosotros) y se promovio a EFECTIVA en 2a publicacion; verificada en vivo con consola limpia. Rutas sensibles siguen 404 (incl. docs/historico/). Backlog: retirar unsafe-inline (post-lanzamiento). Nota operativa: la 1a publicacion quedo detenida en el prompt del mensaje de commit; si el deploy no aparece, revisar que el script haya llegado al push. |
